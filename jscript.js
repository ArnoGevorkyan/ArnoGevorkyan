...
IterateObject(EmojiLib.lib, (v, k) => {
  ...
  v.name = k;
  v.name = v.name.replace(/\+/g, "plus").replace(/\_/g, "-").trim();
...
window.addEventListener("load", function () {
  ...
  function checkHash() {
    if (location.hash) {
      ...
      singleItem
        .querySelector(".icon")
        .setAttribute("class", "icon ec ec-" + iconName);
      singleItem.querySelector("pre").innerHTML =
        '&lt;span class="ec ec-' + iconName + '"&gt;&lt;/span&gt;';
